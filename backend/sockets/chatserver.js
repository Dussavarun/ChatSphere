  ...
  io = new Server(server, {
    cors: {
       origin: process.env.NODE_ENV === 'production'
          ?  ["https://chataa.netlify.app"] :  // since backend should connect with frontend you should use the frontend url to connect from server
      [ "http://localhost:5173"] ,
      methods: ["GET", "POST"],
      credentials: true,
    },
  ...
  io.on("connection", async (socket) => {
    ...
    socket.on("groupmessage", async ({ groupName, sender, message }) => {
      try {
        ...
        console.log(
          `Message sent to group ${groupName} (${roomName}) by ${sender}`
        );
