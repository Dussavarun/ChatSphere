...
import { usechatTypeStore } from "../../backend/store/chatTypeStore";
import socket from "../../backend/sockets/clientsocket.js";
...
import { userSocketstore } from "../../backend/store/userSocketstore.js";
...
const Chatui = () => {
  ...
  useEffect(() => {
    ...
    if (!isConnected && socket) {
      console.log(
        "Socket exists but not connected, attempting reconnection..."
      );
      initSocket();
    }
  }, [isConnected, socket]);
  ...
  useEffect(() => {
    const handleReceivedMessage = (data) => {
      if (userEmail) {
        axios
          .get(`${API_BASE_URL}/message/conversations/${userEmail}`)
          .then((response) =>
  ...
  const handleNewChatSubmit = async (e) => {
    ...
    try {
      ...
      const checkUser = await axios.get(
        `${API_BASE_URL}/user/check/${newRecipientEmail}`
      );
      ...
      try {
        const convResponse = await fetchConversations(userEmail);
        setConversations(convResponse.data.conversations || []);
      } catch (err) {
        console.error("Warning: Could not fetch updated conversations", err);
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-black to-gray-800 text-white">
      ...
      <div className="container mx-auto p-4">
        <div
          className="bg-black/40 backdrop-blur-xl rounded-2xl shadow-2xl overflow-hidden border border-gray-700/50"
          style={{ height: "85vh" }}
        >
            ...
            <div className="w-full md:w-1/3 bg-gradient-to-b from-gray-900/80 to-black/80 border-r border-gray-700/50">
              ...
              >
                ...
                <div className="p-4 border-t border-gray-700/50 bg-black/20">
                  ...
                  {createGroupshow && (
                    <GroupCreate onClose={() => setcreateGroupshow(false)} />
                  )}
                  <GroupsDisplay />
                </div>
                  ...
                  <div className="flex-1 flex items-center justify-center p-8">
                    <div className="text-center max-w-md">
                      ...
                      <p className="text-gray-400 mb-6">
                        Select a conversation from the sidebar or create a new
                        chat below
                      </p>
                      <form
                        className="space-y-4"
